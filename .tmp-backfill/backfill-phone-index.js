...
function initAdmin() {
    ...
    if (!projectId || !clientEmail || !privateKey) {
        throw new Error("Missing Firebase Admin env vars. Required: NEXT_PUBLIC_FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY");
    }
...
async function run() {
    ...
    console.log(`[Backfill] Starting phone index backfill (${APPLY_MODE ? "APPLY" : "DRY-RUN"})`);
    ...
    const resolvedPhoneOwner = new Map();
    let batch = APPLY_MODE ? db.batch() : null;
    let batchOps = 0;
    let committedBatches = 0;
    const flushBatch = async () => {
        if (!APPLY_MODE || !batch || batchOps === 0)
            return;
        await batch.commit();
        committedBatches += 1;
        batch = db.batch();
    ...
    if (conflicts.length > 0) {
        ...
        for (const c of conflicts) {
            console.log(`phone=${c.phone} existingUserId=${c.existingUserId} newUserId=${c.newUserId}`);
        }
    }
}
run().catch((error) => {
