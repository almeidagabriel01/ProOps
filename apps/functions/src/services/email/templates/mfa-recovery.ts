export interface MfaRecoveryEmailData {
  recoverUrl: string;
}

export interface MfaRecoveryEmailContent {
  subject: string;
  html: string;
  text: string;
}

export function renderMfaRecoveryEmail(
  data: MfaRecoveryEmailData,
): MfaRecoveryEmailContent {
  const subject =
    "Recuperar acesso à verificação em dois fatores — ProOps";
...
Este link expira em 30 minutos e só pode ser usado uma vez. Se você não solicitou esta recuperação, ignore este email — nada será alterado na sua conta.
...
<head>
  ...
  <title>Recuperar acesso à verificação em dois fatores — ProOps</title>
</head>
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td align="center" style="padding:0 0 32px;">
                    <a href="${escapeAttr(data.recoverUrl)}"
              ...
              <p style="margin:0 0 24px;font-size:13px;color:#3f3f46;line-height:1.6;word-break:break-all;">
                <a href="${escapeAttr(data.recoverUrl)}" style="color:#3f3f46;text-decoration:underline;">${escapeHtml(data.recoverUrl)}</a>
              </p>
              <p style="margin:0;font-size:13px;color:#71717a;line-height:1.6;">
                Se você não solicitou esta recuperação, pode ignorar este email — nada será alterado na sua conta.
              </p>
            </td>
          </tr>
          <tr>
