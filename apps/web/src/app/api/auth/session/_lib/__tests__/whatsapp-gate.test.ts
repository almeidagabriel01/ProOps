...
describe("decideWhatsappGate", () => {
  ...
  it("skips the gate when the login was completed via the WhatsApp fallback", () => {
    // The WhatsApp factor was already satisfied on the native TOTP screen, so a
    // second WhatsApp challenge must NOT be raised, even if one would require it.
    expect(
      decideWhatsappGate({
        isSuperAdmin: false,
        hasNativeSecondFactor: false,
